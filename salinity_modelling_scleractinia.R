...
geographic.extent.buffer <- extent(x = c(139, 192, -16, 37))
...
sal.historical.mean.data = raster("230620T125922_230601T165519_230321_sos_CMIP6_historical_1985_2015_mean-0p5deg.txt")
sal.126.mean.data = raster("230620T130023_230601T165519_230321_sos_CMIP6_ssp126_2071_2101_mean-0p5deg.txt")
sal.370.mean.data = raster("230620T130124_230601T165519_230321_sos_CMIP6_ssp370_2071_2101_mean-0p5deg.txt")
sal.585.mean.data = raster("230620T130225_230601T165519_230321_sos_CMIP6_ssp585_2071_2101_mean-0p5deg.txt")
...
sal.historical.mean.data.cropped <- crop(sal.historical.mean.data, geographic.extent.buffer)
sal.126.mean.data.cropped <- crop(sal.126.mean.data, geographic.extent.buffer)
sal.370.mean.data.cropped <- crop(sal.370.mean.data, geographic.extent.buffer)
sal.585.mean.data.cropped <- crop(sal.585.mean.data, geographic.extent.buffer)
...
# Create RasterStack
sal.historical.mean.data.cropped <- stack(sal.historical.mean.data.cropped)
sal.126.mean.data.cropped <- stack(sal.126.mean.data.cropped)
sal.370.mean.data.cropped <- stack(sal.370.mean.data.cropped)
sal.585.mean.data.cropped <- stack(sal.585.mean.data.cropped)
...
# Simplest way to use 'evaluate'
e1 <- evaluate(xm.126.mean, p=occ.test, a=bg, x=sal.126.mean.data.cropped)
plot(e1, 'ROC')

#####################################
# Salinity mean data SSP scenario 370
xm.370.mean <- maxent(sal.370.mean.data.cropped, occ.train)
plot(sal.370.mean.data.cropped)

# Plot showing importance of each variable
plot(xm.370.mean)

# Response curves
response(xm.370.mean)

# Predict to entire dataset
xm.370.mean.pred <- predict(xm.370.mean, sal.370.mean.data.cropped)
plot(xm.370.mean.pred)

# Testing the model
# Background data
bg <- randomPoints(sal.370.mean.data.cropped, 10000) #background "pseudoabsences"

# Simplest way to use 'evaluate'
e1 <- evaluate(xm.370.mean, p=occ.test, a=bg, x=sal.370.mean.data.cropped)
plot(e1, 'ROC')
...
####################################################################
# # Plot predictions for salinity change data SSP scenario 126 (Fig 4b)
...
####################################################################
# # Plot predictions for salinity change data SSP scenario 585 (Fig 4b)
# 
...
# ylab="Latitude (Degrees)"
# xlab="Longitude (Degrees)"
axis(side=1,at=c(140,150,160,170,180,190),labels=c("140E","150E","160E","170E","180","170W"))
axis(side=2,at=c(-10,0,10,20,30),labels=c("10S","0","10N","20N","30N"))
# legend(cex=0.8)
cex.axis=0.8
mtext("SSP1-2.6", side=3, font =2, adj=1, cex=0.8)
...
##############################################################################################
# Plot predictions for salinity mean data SSP scenario 370 (this scenario is not yet in a Fig)

# PNG File
png(file="sos_mean_370_marshall_prediction.png", width = 1230, height = 1200, res = 300)

plot(xm.370.mean.pred, col = cols(100),xaxt = "n", yaxt = "n")
...
# legend(cex=0.8)
cex.axis=0.8
mtext("SSP3-7.0", side=3, font =2, adj=1, cex=0.8)
...
# EPS File
postscript("sos_mean_370_marshall_prediction.eps", width = 1230, height = 1200)
